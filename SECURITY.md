...
| ETW 系统事件监听 | 短时监听 D2R 是否成功读取 `WEB_TOKEN`，作为 Token 直启和 Battle.net 启动的登录就绪信号之一，与 TCP 检测并行，任一命中即停止监听。 | 监听按注册表值名和目标游戏 PID 匹配，不用于收集其他应用的注册表内容。 |
| TCP 连接查询 | 查询 IPv4、IPv6 TCP 表，连续两次检测到目标 D2R PID 已建立远端端口为 `1119` 或 `443` 的连接时，作为启动流程的联网就绪信号；`443` 为兼容性兜底，连接建立不保证服务端认证已完成。 | 仅使用目标 PID、连接状态和远端端口判断，不抓取或解析网络数据包内容。 |
| 文件系统读写 | 保存全局配置、账号运行快照、游戏 `Settings.json`、日志、统计数据库和音频识别数据；准备新的识别 Mod。 | 数据保存在本机；Mod 工具写入用户选择的新输出目录，不覆盖源 Mod。 |
| 系统全局热键 | 通过 RegisterHotKey 注册主面板、账号聚焦和自动跟房组合键。 | 录制及自动输入期间暂停注册；不记录普通按键，注册占用会报错。 |
