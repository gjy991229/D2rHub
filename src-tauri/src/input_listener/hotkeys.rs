...
use windows::Win32::UI::WindowsAndMessaging::{GetMessageW, PeekMessageW, PostThreadMessageW, MSG, PM_NOREMOVE, PM_REMOVE, WM_APP, WM_HOTKEY, WM_QUIT};
...
enum Request {
    Refresh { generation: u64, keys: Vec<String>, reply: Option<Reply> },
...
pub(super) fn initialize(app: AppHandle) -> Result<(), String> {
    ...
    if let Some(worker) = slot.take() { let _ = worker.handle.join(); }
        ...
        while unsafe { GetMessageW(&mut msg, HWND::default(), 0, 0) }.0 > 0 {
            ...
            if msg.message == WORK_MESSAGE {
                while let Ok(request) = receiver.try_recv() {
                    match request {
                        Request::Refresh { generation, keys, reply } => {
                        ...
                        Request::Validate(keys, reply) => {
                            ...
                            } else { registrations.validate(&keys) };
    ...
    *slot = Some(Worker { sender, thread_id, cancelled, handle });
    ...
    crate::logger::log_msg("INFO", "Shortcut", "RegisterHotKey / WM_HOTKEY 系统热键服务已启动");
    Ok(())
}
...
fn send(request: Request) -> Result<bool, String> {
    ...
    let Some(worker) = slot.as_ref() else { return Ok(false); };
    if worker.handle.is_finished() || worker.cancelled.load(Ordering::Acquire) {
        return Err("系统热键线程已停止".into());
    }
    worker.sender.send(request).map_err(|_| "系统热键线程已退出")?;
    unsafe { PostThreadMessageW(worker.thread_id.load(Ordering::Acquire), WORK_MESSAGE, WPARAM(0), LPARAM(0)) }
...
fn refresh_request(reply: Option<Reply>) -> Request {
    ...
    Request::Refresh { generation, keys: registered_shortcuts(), reply }
}

pub(super) fn refresh() {
    if let Err(error) = send(refresh_request(None)) {
        if let Some(app) = APP_HANDLE.lock().ok().and_then(|app| app.clone()) { report_error(&app, &error); }
    }
}

pub(super) fn refresh_sync() -> Result<(), String> {
    let (tx, rx) = mpsc::sync_channel(1);
    if !send(refresh_request(Some(tx)))? { return Ok(()); }
    rx.recv_timeout(TIMEOUT).map_err(|_| "更新系统热键超时".to_string())?
}

pub(super) fn validate(keys: Vec<String>) -> Result<(), String> {
    for key in &keys { if !key.trim().is_empty() { parse(key)?; } }
    let (tx, rx) = mpsc::sync_channel(1);
    if !send(Request::Validate(keys, tx))? { return Ok(()); }
    rx.recv_timeout(TIMEOUT).map_err(|_| "检查系统热键占用超时".to_string())?
}

fn blocked() -> bool { SHORTCUT_CAPTURE_ACTIVE.load(Ordering::Acquire) || is_suspended() }
pub(crate) fn is_suspended() -> bool { SUSPENSIONS.load(Ordering::Acquire) != 0 }
...
impl Drop for Pause {
    fn drop(&mut self) {
        if SUSPENSIONS.fetch_sub(1, Ordering::AcqRel) == 1 { refresh(); }
    }
}
...
pub(super) fn shutdown() {
    ...
    if let Some(worker) = worker {
        worker.cancelled.store(true, Ordering::Release);
        let posted = unsafe { PostThreadMessageW(worker.thread_id.load(Ordering::Acquire), WM_QUIT, WPARAM(0), LPARAM(0)) };
        if posted.is_ok() || worker.handle.is_finished() { let _ = worker.handle.join(); }
    }
}

fn parse(value: &str) -> Result<(u32, u32), String> {
    let normalized = crate::capabilities::room_automation::canonicalize_shortcut(value)
        .map_err(|error| format!("快捷键 {value} 无效：{error}"))?.to_ascii_lowercase();
    let mut key = normalized.as_str();
    let mut modifiers = 0x4000; // MOD_NOREPEAT
    for (prefix, flag) in [("ctrl+", 2), ("alt+", 1), ("shift+", 4)] {
        if let Some(rest) = key.strip_prefix(prefix) { modifiers |= flag; key = rest; }
    }
    let vk = (0x08..=0xFE).find(|vk| vk_to_key_string(*vk).eq_ignore_ascii_case(key))
        .ok_or_else(|| format!("无法注册快捷键 {value}：不支持的按键"))?;
    if vk == 0x7B { return Err("F12 是 Windows 调试器保留键，请选择其他快捷键".into()); }
    Ok((modifiers, vk))
}

#[derive(Default)]
struct Registrations { keys: HashMap<i32, String>, next_id: i32 }
impl Registrations {
    fn add(&mut self, key: &str) -> Result<i32, String> {
        ...
        if self.keys.is_empty() {
            ...
            let mut msg = MSG::default();
            while unsafe { PeekMessageW(&mut msg, HWND::default(), WM_HOTKEY, WM_HOTKEY, PM_REMOVE) }.as_bool() {}
            self.next_id = 0;
        }
        ...
        self.next_id += 1;
        if self.next_id > 0xBFFF { return Err("系统热键标识已用尽，请重启 D2RHub".into()); }
        unsafe { RegisterHotKey(HWND::default(), self.next_id, HOT_KEY_MODIFIERS(modifiers), vk) }
    ...
    fn validate(&mut self, keys: &[String]) -> Result<(), String> {
        ...
        for key in keys.iter().filter(|key| !key.trim().is_empty()) {
            ...
            if self.keys.values().any(|current| *current == key) { continue; }
            match self.add(&key) {
        ...
        for id in temporary {
            if let Err(error) = self.remove(id) { result = Err(error); }
        }
        result
    }
    fn replace(&mut self, keys: &[String]) -> Result<(), String> {
        let desired: HashSet<_> = keys.iter().map(|key| key.trim().to_ascii_lowercase())
            .filter(|key| !key.is_empty()).collect();
        let retired: Vec<_> = self.keys.iter().filter(|(_, key)| !desired.contains(*key)).map(|(id, _)| *id).collect();
        let mut errors = Vec::new();
        for id in retired { if let Err(error) = self.remove(id) { errors.push(error); } }
        for key in desired {
            if !self.keys.values().any(|current| *current == key) {
                if let Err(error) = self.add(&key) { errors.push(error); }
            }
        }
...
impl Drop for Registrations {
    fn drop(&mut self) {
        for id in self.keys.keys() { let _ = unsafe { UnregisterHotKey(HWND::default(), *id) }; }
    }
}
