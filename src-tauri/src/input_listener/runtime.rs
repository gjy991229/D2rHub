...
impl Drop for Completion {
    fn drop(&mut self) {
        ...
        if let Ok(mut sender) = EVENT_TX.lock() {
            sender.take();
        }
        let _ = self.0.send(());
    }
...
unsafe extern "system" fn mouse(code: i32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    if code >= 0 && matches!(wparam.0 as u32, WM_LBUTTONDOWN | WM_RBUTTONDOWN) {
        ...
        if event.dwExtraInfo != crate::infrastructure::physical_input::INPUT_TAG {
            emit(if wparam.0 as u32 == WM_LBUTTONDOWN {
                "MouseLeft"
            } else {
                "MouseRight"
            });
    ...
    tauri::async_runtime::spawn_blocking(|| {
        loop {
            ...
            if let Err(error) = reconcile() {
                crate::logger::log_msg("ERROR", "PetInput", &error);
            }
...
fn reconcile() -> Result<(), String> {
    ...
    if slot
        .as_ref()
        .is_some_and(|worker| needed() && !worker.stopping && !worker.handle.is_finished())
    {
        return Ok(());
    }
    if let Some(worker) = slot.as_mut() {
        ...
        if !worker.handle.is_finished() {
            ...
            if matches!(
                worker.done.recv_timeout(TIMEOUT),
    ...
    let handle = std::thread::Builder::new()
        ...
        .spawn(move || {
            ...
            let install = || -> Result<(Hook, Hook), String> {
                let keyboard = Hook(
                    unsafe {
                        SetWindowsHookExW(WH_KEYBOARD_LL, Some(keyboard), HINSTANCE::default(), 0)
                    }
                    .map_err(|error| format!("安装桌宠键盘监听失败：{error}"))?,
                );
                let mouse = Hook(
                    unsafe { SetWindowsHookExW(WH_MOUSE_LL, Some(mouse), HINSTANCE::default(), 0) }
            ...
            if stop.load(Ordering::Acquire) || ready_tx.send(Ok(())).is_err() {
                return;
            }
