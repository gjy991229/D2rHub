...
mod windows {
    ...
    fn process_creation_time(pid: u32) -> Option<u64> {
        ...
        if process.0.is_null() || unsafe { WaitForSingleObject(process.0, 0) } != 258 {
            return None;
        }
    ...
    pub(crate) fn restore_instances(state: &crate::state::SharedState) {
        let inherited = INHERITED_INSTANCES
            .lock()
            .unwrap_or_else(|error| error.into_inner())
            .take();
        if let Some(instances) = inherited {
            for instance in instances {
                if process_creation_time(instance.pid) != Some(instance.creation_time) {
                    continue;
                }
                if let Some(arguments) = &instance.mod_args {
                    ...
                    state
                        .multi_instance()
                        .instances()
                        .record_discovered(&instance.account_id, instance.pid);
                }
            }
        }
    }
    ...
    impl PreparedRestart {
        pub(crate) fn prepare(app: &tauri::AppHandle) -> Result<Self, String> {
            ...
            let worker = std::thread::Builder::new()
                .name("restart-handoff".to_string())
                .spawn(move || {
    ...
    fn prepare_child(
        ...
        let event =
            Handle(unsafe { CreateEventW(std::ptr::null_mut(), 1, 0, wide(&event_name).as_ptr()) });
        if event.0.is_null() {
            return Err(format!(
                "无法准备重启交接: {}",
        ...
        let executable =
            std::env::current_exe().map_err(|error| format!("无法定位 D2RHub: {error}"))?;
        let child = Command::new(executable)
            ...
            .spawn()
            .map_err(|error| format!("无法启动新进程，原模式已保留: {error}"))?;
        ...
        if pending.cancelled.load(Ordering::Acquire)
            || remaining.is_zero()
            || unsafe {
                WaitForSingleObject(event.0, remaining.as_millis().min(u32::MAX as u128) as u32)
            } != WAIT_OBJECT_0
        {
        ...
        if child
            ...
            .ok_or("重启准备已取消")?
            .try_wait()
            .map_err(|error| format!("无法确认新进程状态: {error}"))?
            .is_some()
        {
    ...
    pub(crate) fn wait_for_predecessor() -> Result<(), String> {
        ...
        if parent.0.is_null() {
            return Err("无法等待原进程退出".to_string());
        }
