...
- **按需加工 Mod 功能**：D2RHub 内置独立的 `d2r-audio-mod.exe` 生成器。r22 可分别选择“声纹识别”和“局内房间工具”，以后新增功能也沿用独立功能组；每组在清单中记录自己的配方版本和参数指纹。把 r22 成品作为下一次来源时，版本、覆盖、类别、增益、目录与全部声纹文件都一致才会直接复用，未变化的 FLAC 不重新编码或覆盖。局内工具包含常驻的“下一局 / 创建房间 / 加入房间”紧凑工具栏，并严格复用 MDK 的房间面板消息链与下一局动作。加工先复制源 Mod 到事务目录，只在新产物中更新所选组；源 Mod 与已有同名输出均不覆盖，失败时不提交半成品。
- **明确的 r22 起点**：r21 及更老加工产物没有独立功能组和参数指纹，D2RHub 不再把它们识别为可用声纹 Mod、更新目标或增量来源。用户需要从原版或当初未经加工的原始 Mod 生成一个新名称的 r22 产物；从此以后，未变化的功能组才能被可靠识别和复用。
- **局内后台换房**：D2RHub 会为原生 `CfgChat` 的空闲第二键位安装 F13，并逐文件备份 `.key/.keyo`；启用后常驻监视两个存档目录，新账号或新角色生成键位文件时会在 50ms 轮询内自动补齐，初始化阶段再次覆盖也会重新校验。自动流程先以 `Esc` 打开 Mod 的无操作安全焦点，再连续发送两次左/右选择创建或加入入口，只有选中入口后确认键才会生效；即使方向消息漏掉也不会点中暂停菜单的真实按钮。原生表单打开后发送 F13 建立会屏蔽技能快捷键的真实文本输入态，随后填写房名、Tab 切到密码并提交。小号阶段默认仍由快捷键手动开始；也可勾选 2–60 秒延迟自动跟进（默认 5 秒）。所有参与账号必须使用包含 r22“局内房间工具”功能组的 Mod。
...
- A bundled but independently maintained `d2r-audio-mod.exe` generator with independently selectable and versioned r22 feature groups. Audio telemetry and in-game room tools can be added separately; a verified unchanged audio group is copied without re-encoding its FLAC assets. Augmentation remains transactional and never mutates the source or overwrites an existing output.
