...
- **一键准备识别 Mod**：D2RHub 内置独立的 `d2r-audio-mod.exe` 生成器。无论从原版还是现有 Mod 准备，用户都需要为新 Mod 命名；软件随后生成新 Mod、复核清单并安全配置账号启动参数。r21 配方会额外加入常驻的局内“下一局 / 创建房间 / 加入房间”工具栏，三个按钮缩至 0.30 倍并在右上角紧凑排列；工具仍严格复用 MDK 同一 `0.1s` 时点的房间面板消息链与 `CharacterSelect:LoadCharacter:2` 下一局动作，并在两套 PausePanel 加入不可见的安全键盘入口。加工先复制源 Mod 到事务目录，只在新产物中结构化更新必需表格字段和具名布局节点；源 Mod及已有同名输出均不覆盖，失败时不提交半成品。游戏、源 Mod 与输出目录支持中文、空格及 Windows 允许的特殊字符；源 Mod 数据表兼容 UTF-8、UTF-16、GBK/GB18030 与常见 Windows ANSI 编码，JSON 资源兼容常见 JSON5 写法，0 字节 FLAC 静音占位也会保持静音。
- **局内后台换房**：D2RHub 会为原生 `CfgChat` 的空闲第二键位安装 F13，并逐文件备份 `.key/.keyo`；启用后常驻监视两个存档目录，新账号或新角色生成键位文件时会在 50ms 轮询内自动补齐，初始化阶段再次覆盖也会重新校验。自动流程先以 `Esc` 打开 Mod 的无操作安全焦点，再连续发送两次左/右选择创建或加入入口，只有选中入口后确认键才会生效；即使方向消息漏掉也不会点中暂停菜单的真实按钮。原生表单打开后发送 F13 建立会屏蔽技能快捷键的真实文本输入态，随后填写房名、Tab 切到密码并提交。小号阶段默认仍由快捷键手动开始；也可勾选 2–60 秒延迟自动跟进（默认 5 秒），届时直接复用后台加入流程，不模拟全局快捷键、不要求主号继续保持前台。r21 配置不再保存鼠标坐标、后台点击策略或旧大厅超时字段，只保留已经实测稳定的键盘时序。所有参与账号必须启用同一代 r21 加工 Mod。
...
- A bundled but independently maintained `d2r-audio-mod.exe` generator for creating a minimal Mod or augmenting an existing unpacked Mod without coupling either codebase or data model. Recipe r21 adds a compact 0.30-scale Next/Create/Join toolbar, preserves MDK's same-tick `0.1s` native room message chain and `CharacterSelect:LoadCharacter:2` action, and adds a no-op keyboard focus hub in both PausePanel variants. Automated input opens that safe hub with Esc, repeats Left/Right to select the hidden Create/Join entry, and confirms only after a valid selection. The follower phase remains manual by default or can continue automatically after a configurable 2–60 second delay (default 5 seconds) without synthesizing a global shortcut. Augmentation is transactional: it copies the source, structurally patches only required tables and named layout nodes in the new output, never mutates the source or overwrites an existing output, and commits only after validation succeeds.
