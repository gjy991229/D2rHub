...
- **局内按钮显示开关**：在“设置 → 可选功能 → Mod 管理”的具体 Mod 条目中切换“显示游戏内右上角按钮”。先关闭使用该 Mod 的游戏，D2RHub 直接修改 HUD 工具栏入口，无需重新加工，下次启动游戏生效。隐藏后不能鼠标点击右上角按钮，但 `Esc → 左/右两次 → 回车` 与自动跟房保留；共用同一 Mod 的账号一起生效，后续增补加工继承该选择。
