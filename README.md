...
- **v4.2 实机探针**：符文只标记 HD 掉落状态机的 `Flippy` 状态；地点标记持续场景环境音。本轮只覆盖罗格营地与黑色荒地，验证通过后再扩展全部地图。
...
4. 在“设置中心 → 自动化”选择完整的 jcy.mpq，生成 `D2RHubAudioProbeV42`，启动参数使用 `-mod D2RHubAudioProbeV42 -txt`。
...
- A v4.2 field probe for 33 rune drop-animation IDs plus Rogue Encampment and Black Marsh continuous-ambience IDs, preserving source audio with per-file verification.
...
3. Under **Settings → Automation**, generate `D2RHubAudioProbeV42` from a complete unpacked source mod, then launch with `-mod D2RHubAudioProbeV42 -txt`.
