...
当前实现采用两个必须同时满足的判据：目标 `D2R.exe` PID 成功读取注册表值 `WEB_TOKEN`，并且该 PID 的多开互斥句柄已经成功关闭。Token 流程不再读取 TCP 表、不再等待游戏窗口，也不再依赖 `WEB_TOKEN` 内容发生回写。
...
8. 当前 Token 账号任一必要条件失败时立即停止批次，绝不覆盖共享 Token 启动下一个账号。
...
- 国际服 Token 一键启动已于 2026-08-26 完成实机验证，Token 读取与句柄清除双条件符合预期。国服使用同一 Token 直接启动路径，理论上生效，但仍应单独做一次实机回归。
