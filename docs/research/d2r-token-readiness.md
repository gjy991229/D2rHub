...
筛选 `TokenETW` 及对应账号的 `Launch` 日志。每次监听使用 session 名关联启动、
目标 PID 绑定、处理线程退出和最终汇总；最终汇总包含主动停止原因。
TCP 1119 先命中明确记录为正常停止，ETW 的 WEB_TOKEN 成功读取判据不变。

- 启动失败：保留 ferrisetw 的原始错误和系统错误码。依赖将创建会话、启用 Provider、
  打开消费句柄封装在同一入口，无法从错误确定具体子步骤时不推断。
- 消费线程：显式持有线程，记录 ProcessTrace 返回错误或意外退出；关闭监听后回收线程。
- 接收情况：统计 Provider 全部事件、ID=7 查询事件、最近事件时间及目标 PID 绑定后的事件数。
  绑定前的成功 WEB_TOKEN 读取仍按 PID 缓存，不受诊断计数影响。
- 解析情况：按 schema / ValueName / Status 统计错误，并保留前三条错误的 PID、事件 ID、版本和原因。
  解析错误属于整个 Provider，不能直接认定来自目标游戏。
- 读取情况：记录成功读取 PID，以及最多 32 组 WEB_TOKEN 查询的 PID、十六进制 NTSTATUS 和次数。
  不读取或记录 Token 内容，不输出其他注册表值名和事件原始载荷。
- 会话健康：停止前只读查询 EventsLost、RealTimeBuffersLost、LogBuffersLost 和 BuffersWritten。
  查询失败记录 Win32 错误码，不把未知丢失数当成零。

零事件、未匹配、解析失败、消费线程退出是不同观察结果。汇总保留证据和有限结论，
不会仅凭零事件断言权限故障，或仅凭其他 PID 命中断言由辅助进程代读。
