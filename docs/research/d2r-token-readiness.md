...
批量启动通常要求当前账号启动成功且互斥句柄已清除，才会继续启动下一个账号。
若 60 秒内两种登录信号均未命中，仅当本次启动的原游戏进程仍存活、且互斥句柄已清除时，
允许以“登录状态未确认”继续队列。此时结果为 `success=false`、`login_unconfirmed=true`，
不表示登录成功；任务摘要与迷你模式会单独提示。进程句柄在等待前取得并核对可执行文件路径，
防止 PID 复用误判；降级前及返回结果前再次确认存活。进程已退出或无法确认时停止队列，
只有明确确认原进程退出才移除运行记录。
